...
from tests.utils import get_nodes_keys, get_polygons_keys, clean_directory
from tests.mocks import get_test_grid, ConfigMock


CONFIG = ConfigMock()

MOSCOW_FIELDS = Path(r'./data/moscow_fields.csv')
...
class TestGrid(unittest.TestCase):
    ...
    def test_border_test(self):
        ...
        # Arrange
        xgml = Xgml(TEST, CONFIG.mgen)
        xgml.parse(str(CONFIG.mgen.xgml[TEST]))
        grid = Grid(TEST, xgml.nodes, xgml.edges, CONFIG.mgen)
        # Act
    ...
    def test_border_stalin(self):
        ...
        grid = Grid(STALIN, xgml.nodes, xgml.edges, CONFIG.mgen)
        expected = (
    ...
    def test_grid_capturing_test(self):
        ...
        xgml.parse(str(CONFIG.mgen.xgml[TEST]))
        grid = Grid(TEST, xgml.nodes, xgml.edges, CONFIG.mgen)
        path = Path(r'./tmp/{}_{}.xgml'.format(TEST, 0))
        ...
        for i in range(1, self.iterations):
            ...
            for node in nodes.pop().neighbors:
                if node.country == 201:
                    node.capture(101)
                    break
            path = Path(r'./tmp/{}_{}.xgml'.format(TEST, i))
            xgml.save_file(str(path), grid.nodes, grid.edges)
    ...
    def test_get_neighbors_of(self):
        ...
        # act
        result = get_nodes_keys(grid.get_neighbors_of(grid.border_nodes))
        # assert
        self.assertCountEqual(result, expected)

    def _test_grid_capturing_moscow(self):
        """Выполняется захват в графе Москвы"""
        xgml = Xgml(MOSCOW, CONFIG.mgen)
        xgml.parse(str(CONFIG.mgen.xgml[MOSCOW]))
        grid = Grid(MOSCOW, xgml.nodes, xgml.edges, CONFIG.mgen)
        path = Path(r'./tmp/{}_{}.xgml'.format(MOSCOW, 0))
        xgml.save_file(str(path), grid.nodes, grid.edges)
        # Act
        self.fail()
...
class TestNode(unittest.TestCase):
    ...
    def test_node_triangles(self):
        ...
        # act
        result = get_polygons_keys(grid.node(30).triangles)
        # assert
        self.assertCountEqual(result, expected)
    ...
    def test_neighbors_sorted(self):
        ...
        # act
        result = get_nodes_keys(grid.node(28).neighbors_sorted)
        # assert
